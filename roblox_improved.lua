...
local TixSettings = {
    ...
    SpinSpeed = 15,
    TargetPart = "Head",
    SmartToggleEnabled = true, -- NOVO: Smart Toggle ativado por padrão
}
...
-- ══════════════════════════════════════════════════════════════
-- SMART TOGGLE SYSTEM (Anti-Detection)
-- ══════════════════════════════════════════════════════════════
...
local function isAdmin(player)
    ...
    -- Verificar se tem badges de admin/moderador
    ...
    -- Verificar se tem permissões (pode usar RemoteEvents admin)
    pcall(function()
        if player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
            -- Verificar se consegue fazer ações de admin (heurística)
...
local function smartToggle()
    ...
    if anyAdminNearby and not ScriptDisabled then
        -- ADMIN DETECTADO: Desativar features óbvias
        ScriptDisabled = true
        ...
        -- Manter apenas: ESPName, ESPDistance, ESPHealth (menos óbvio)
        
        -- Log silencioso (sem aparecer na tela)
        print("[REDZ] ⚠️ Admin detectado. Features desativadas automaticamente.")
    elseif not anyAdminNearby and ScriptDisabled then
        -- Admin saiu: Reativar (opcional - pode deixar desativado por segurança)
...
RunService.RenderStepped:Connect(function()
    if ScriptDisabled then return end -- Pular frame se admin foi detectado
    ...
    -- ══════════════════════════════════════════════════════════════
    -- IMPROVED ESP SKELETON - MAIS BONITO E SUAVE
    -- ══════════════════════════════════════════════════════════════
    ...
    if anyESP then
        ...
        for _, p in pairs(targets) do
            ...
            if not visualCache[char] then
                ...
                -- Criar 14 linhas para skeleton (R15)
                for i = 1, 14 do
                    local l = Drawing.new("Line")
                    l.Thickness = 2.5 -- Mais grossa (era 1.5)
                    l.Transparency = 0.85 -- Transparência suave
                    l.Visible = false
            ...
            if TixSettings.ESPSkeleton and isAlive then
                ...
                -- Armazenar posições de viewport para suavizar animação
                local bonePositions = {}
                    ...
                    if partA and partB then
                        ...
                        if visA and visB then
                            ...
                            if line then
                                ...
                                -- Animação dinâmica de cor com pulsação sutil
                                -- Muda gradualmente entre cor base e versão mais brilhante
                                local time = tick() % 2 -- Loop a cada 2 segundos
                                local pulse = math.sin(time * math.pi) * 0.2 + 0.8 -- Varia entre 0.6 e 1.0
                                line.Transparency = 0.85 - (pulse * 0.15)
                            end
                        end
                    end
                end
