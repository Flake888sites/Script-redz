-- =============================================
-- REDZ XITER V2 - Script Unificado
-- =============================================
...
local function AddToggle(text, settingKey)
    ...
    Label.TextColor3 = Cores.Primary
    Label.TextSize = 14
    ...
    Status.TextColor3 = Cores.OffText
    Status.TextSize = 13
...
local function AddSlider(text, settingKey, min, max, default)
    ...
    container.BackgroundColor3 = Cores.CardBg
    ...
    SliderBtn.BackgroundColor3 = Cores.Primary
    SliderBtn.Text = ""
...
local function AddSelector(text, settingKey, options)
    ...
    Status.Font = Enum.Font.GothamBold
    Status.TextColor3 = Cores.Primary
    Status.TextSize = 13
    Status.TextXAlignment = Enum.TextXAlignment.Right

    local currentIndex = 1
    for i, v in ipairs(options) do
        if v == TixSettings[settingKey] then
            currentIndex = i
            break
        end
    end
...
RunService.RenderStepped:Connect(function()
    ...
    for _, p in pairs(Players:GetPlayers()) do
        if p ~= LocalPlayer
            and p.Character
            and p.Character:FindFirstChild(TixSettings.TargetPart)
