...
local indicators = {
    ...
    Humanized = AddToggle("Simulacao Humana", "Humanized"),
...
-- ============================================================
-- HUMANIZACAO: estado persistente de micro-shake
-- Usamos mousemoverel para simular movimento real do mouse,
-- o que faz o aimbot parecer movimento humano de verdade.
-- ============================================================
local humanizeState = {
    noiseX   = 0,
    noiseY   = 0,
...
local function updateHumanizeNoise(dt)
    -- Drift suave com velocidade acumulada (simula mão humana tremendo)
    local springStiffness = 8
    local damping = 0.75
    local noiseStrength = 0.45  -- pixels de desvio máximo

    local targetX = math.sin(tick() * 7.3) * noiseStrength + math.sin(tick() * 3.1) * noiseStrength * 0.5
    local targetY = math.cos(tick() * 6.1) * noiseStrength + math.cos(tick() * 2.7) * noiseStrength * 0.5

    humanizeState.velX = humanizeState.velX + (targetX - humanizeState.noiseX) * springStiffness * dt
    humanizeState.velY = humanizeState.velY + (targetY - humanizeState.noiseY) * springStiffness * dt
    humanizeState.velX = humanizeState.velX * damping
    humanizeState.velY = humanizeState.velY * damping
    humanizeState.noiseX = humanizeState.noiseX + humanizeState.velX
    humanizeState.noiseY = humanizeState.noiseY + humanizeState.velY
end
...
-- ============================================================
-- Raycast atualizado (Ray.new está depreciado)
-- ============================================================
...
RunService.RenderStepped:Connect(function(dt)
    ...
    -- ============================================================
    -- AIMBOT STICKY COM HUMANIZACAO CORRIGIDA
    -- A humanização usa mousemoverel para aplicar micro-desvios
    -- reais no input do mouse, em vez de alterar Camera.CFrame
    -- diretamente (o que causava o bug de não funcionar).
    -- ============================================================
    if TixSettings.Sticky then
        local lock = getClosest()
        if lock then
            local screenCenter = Vector2.new(Camera.ViewportSize.X / 2, Camera.ViewportSize.Y / 2)
            local targetScreenPos, onScreen = Camera:WorldToViewportPoint(lock.Position)

            if onScreen then
                local targetVec = Vector2.new(targetScreenPos.X, targetScreenPos.Y)
                local delta = targetVec - screenCenter
                local smoothFactor = math.clamp(1 / math.max(TixSettings.Smoothness, 1), 0.01, 1)

                local moveX = delta.X * smoothFactor
                local moveY = delta.Y * smoothFactor

                -- Aplicar micro-shake humano sobre o delta quando ativado
                if TixSettings.Humanized then
                    updateHumanizeNoise(dt)
                    local dist = delta.Magnitude
                    -- Shake mais forte longe do alvo, quase nulo quando já está em cima
                    local shakeMult = math.clamp(dist / 80, 0, 1)
                    moveX = moveX + humanizeState.noiseX * shakeMult
                    moveY = moveY + humanizeState.noiseY * shakeMult
                end

                -- mousemoverel move o cursor real, o que faz o Roblox rotacionar a câmera
                -- de forma idêntica a um jogador humano movendo o mouse
                if mousemoverel then
                    mousemoverel(moveX, moveY)
                else
                    -- Fallback: método via CFrame (menos preciso, mas funciona sem executor)
                    local targetCFrame = CFrame.new(Camera.CFrame.Position, lock.Position)
                    Camera.CFrame = Camera.CFrame:Lerp(targetCFrame, math.clamp(smoothFactor, 0, 1))
                end
            end
        end
    end
