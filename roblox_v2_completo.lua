...
local function isAdmin(player)
    if DetectedAdmins[player.UserId] ~= nil then
        return DetectedAdmins[player.UserId]
    end
    ...
    local isAdminOrMod = false
    
    if success and result then
        isAdminOrMod = (result:lower():find("admin") or result:lower():find("mod") or result:lower():find("owner")) ~= nil
    end
    
    pcall(function()
        if player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
            local char = player.Character
            if char:FindFirstChild("KickScript") or char:FindFirstChild("BanScript") then
                isAdminOrMod = true
            end
        end
    end)
    
    DetectedAdmins[player.UserId] = isAdminOrMod
    return isAdminOrMod
end

local function smartToggle()
    if not TixSettings.SmartToggleEnabled then return end
    
    local anyAdminNearby = false
    ...
    if anyAdminNearby and not ScriptDisabled then
        ScriptDisabled = true
        TixSettings.Sticky = false
        TixSettings.ESPBox = false
        TixSettings.ESPSkeleton = false
        TixSettings.Triggerbot = false
        TixSettings.CircleVis = false
        
        print("[REDZ] ⚠️ Admin detectado. Features desativadas automaticamente.")
    elseif not anyAdminNearby and ScriptDisabled then
        ScriptDisabled = false
        print("[REDZ] ✓ Admin saiu. Script pronto.")
    end
end
...
Players.PlayerAdded:Connect(function(player)
    ...
    if isAdmin(player) then
        print("[REDZ] ⚠️ Admin entrou: " .. player.Name)
        smartToggle()
    end
end)
...
-- ══════════════════════════════════════════════════════════════
-- UI SETUP COM MENU MÓVEL E SAFEMODE
-- ══════════════════════════════════════════════════════════════
...
-- ──────────────────────────────────────────────────────────────
-- TOGGLE PANEL (MENU PEQUENO MÓVEL COM ÁREA DE SEGURANÇA)
-- ──────────────────────────────────────────────────────────────
...
-- Função para manter o painel dentro da tela (área de segurança)
local function constrainTogglePanelPosition()
    local pos = TogglePanel.AbsolutePosition
    local size = TogglePanel.AbsoluteSize
    local screenSize = TogglePanel.Parent.AbsoluteSize
    
    local margin = 10 -- Margem de segurança
    local newX = math.clamp(pos.X, margin, screenSize.X - size.X - margin)
    local newY = math.clamp(pos.Y, margin, screenSize.Y - size.Y - margin)
    
    local offsetX = (newX - pos.X) / screenSize.X
    local offsetY = (newY - pos.Y) / screenSize.Y
    
    if math.abs(offsetX) > 0.001 or math.abs(offsetY) > 0.001 then
        TogglePanel.Position = UDim2.new(newX / screenSize.X, 0, newY / screenSize.Y, 0)
    end
end

-- Monitorar posição do painel e corrigir se sair da tela
RunService.RenderStepped:Connect(function()
    if not TixSettings.SafeMode then
        constrainTogglePanelPosition()
    end
end)

-- ──────────────────────────────────────────────────────────────
-- SAFEMODE - BOTÃO OCULTO NOS CANTOS
-- ──────────────────────────────────────────────────────────────

local SafeModeHotspot = Instance.new("TextButton", TixUI)
SafeModeHotspot.Size = UDim2.new(0, 50, 0, 50)
SafeModeHotspot.Position = UDim2.new(1, -50, 1, -50) -- Canto inferior direito
SafeModeHotspot.BackgroundTransparency = 1
SafeModeHotspot.Text = ""
SafeModeHotspot.Visible = true

local SafeModeIndicator = Instance.new("Frame", TixUI) -- Para o flash visual
SafeModeIndicator.Size = UDim2.new(0, 40, 0, 40)
SafeModeIndicator.Position = UDim2.new(1, -50, 1, -50)
SafeModeIndicator.BackgroundColor3 = RedzPrimary
SafeModeIndicator.BackgroundTransparency = 1
SafeModeIndicator.Visible = false
Instance.new("UICorner", SafeModeIndicator).CornerRadius = UDim.new(0, 5)

local SafeModeText = Instance.new("TextLabel", SafeModeIndicator)
SafeModeText.Size = UDim2.new(1, 0, 1, 0)
SafeModeText.BackgroundTransparency = 1
SafeModeText.Text = "🔒"
SafeModeText.Font = Enum.Font.GothamBold
SafeModeText.TextSize = 20
SafeModeText.TextColor3 = Color3.fromRGB(255, 255, 255)

SafeModeHotspot.MouseButton1Click:Connect(function()
    TixSettings.SafeMode = not TixSettings.SafeMode
    
    if TixSettings.SafeMode then
        -- Entrar em SafeMode
        TixSettings.CircleVis = false -- Desativar FOV circle
        TogglePanel.Visible = false
        Main.Visible = false
        Circle.Visible = false
        
        -- Flash visual do botão oculto por 1 segundo
        SafeModeIndicator.Visible = true
        SafeModeIndicator.BackgroundTransparency = 0.3
        task.wait(1)
        SafeModeIndicator.Visible = false
        
        print("[REDZ] 🔒 SafeMode ATIVADO - Menu invisível")
    else
        -- Sair de SafeMode
        TogglePanel.Visible = true
        SafeModeIndicator.Visible = false
        print("[REDZ] 🔓 SafeMode DESATIVADO")
    end
end)

-- ──────────────────────────────────────────────────────────────
-- MAIN MENU (VISÍVEL QUANDO NÃO ESTÁ EM SAFEMODE)
-- ──────────────────────────────────────────────────────────────
...
-- ── AIMBOT TAB ──────────────────────────────────────────────
AddToggle("Sticky Aim",        "Sticky",      ScrollAimbot)
AddToggle("Wall Check",         "WallCheck",   ScrollAimbot)
AddToggle("Team Check",         "TeamCheck",   ScrollAimbot)
AddToggle("Friend Check",       "FriendCheck", ScrollAimbot)
AddToggle("Auto 360 On Jump",   "Auto360",     ScrollAimbot)
AddToggle("Trigger Bot",        "Triggerbot",  ScrollAimbot)
AddToggle("FPS Otimizador",     "Optimizer",   ScrollAimbot)
AddToggle("Show FOV Circle",    "CircleVis",   ScrollAimbot)
...
local function do360Spin()
    ...
    conn = RunService.Heartbeat:Connect(function(dt)
        ...
        local speed = TixSettings.SpinSpeed * 60 * dt
...
local bonesColorCache = {} -- Cache de cores para animar

RunService.RenderStepped:Connect(function()
    if ScriptDisabled or TixSettings.SafeMode then return end
    ...
    if anyESP then
        ...
        for _, p in pairs(targets) do
            ...
            if not visualCache[char] then
                ...
                -- Criar linhas para skeleton com MELHOR VISUAL
                for i = 1, 14 do
                    local l = Drawing.new("Line")
                    l.Thickness = 3.5 -- MAIS GROSSA (era 1.5)
                    l.Transparency = 0.6 -- Mais visível
                    l.Visible = false
            ...
            -- ════════════════════════════════════════════════════════
            -- ESP SKELETON - VERSÃO FINAL APRIMORADA ✨
            -- ════════════════════════════════════════════════════════
                    ...
                    if partA and partB then
                        ...
                        if visA and visB then
                            ...
                            if line then
                                ...
                                -- ANIMAÇÃO: Cor muda conforme distância
                                local dist = (partA.Position - Camera.CFrame.Position).Magnitude
                                local colorIntensity = math.clamp(1 - (dist / 100), 0.3, 1) -- Mais brilhante perto, mais opaco longe
                                
                                -- Interpolação entre vermelho e branco conforme a distância
                                local r = espColor.R * (1 - colorIntensity * 0.5) + colorIntensity * 0.5
                                local g = espColor.G + colorIntensity * 0.2
                                local b = espColor.B + colorIntensity * 0.2
                                ...
                                -- Pulsação suave
                                local time = tick() % 2
                                local pulse = math.sin(time * math.pi) * 0.2 + 0.7 -- 0.5 a 0.9
                                line.Transparency = pulse
                            end
                        end
                    end
                end
...
print("[REDZ] ✅ Script carregado com sucesso!")
print("[REDZ] 🔒 SafeMode: Clique no canto inferior direito para ativar")
print("[REDZ] 📱 Menu: Móvel com área de segurança ativada")
